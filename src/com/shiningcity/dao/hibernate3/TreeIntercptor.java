package com.shiningcity.dao.hibernate3;
...
public class TreeIntercptor extends EmptyInterceptor implements
	...
	private static final Logger log = Logger.getLogger(TreeIntercptor.class);
	...
	private boolean updateParent(HibernateTree<?> tree,
		...
		if (log.isDebugEnabled()) {
			log.debug(String.format("update Tree %s, id=%s, "
					+ "pre-parent id=%s, curr-parent id=%s", new Object[] {
					beanName, tree.getId(),
		...
		if (currParent != null) {
			...
			log.debug(String.format("current node span=%s", span));
			...
			log.debug(String.format("current parent lft=%s rgt=%s", currParentLft,
					currParentRgt));
			...
			session.createQuery(hql2).setInteger("parentRgt", currParentRgt)
					.executeUpdate();
			log.debug(String.format("vacated span hql: %s, %s, parentRgt=%s", new Object[] {
			...
			currParentRgt = ((Number) session.createQuery(hql).uniqueResult())
					.intValue();
			currParentRgt++;
			log.debug(String.format("max node left=%s", currParentRgt));
		}
		...
		int span = nodeRgt - nodeLft + 1;
		if (log.isDebugEnabled()) {
			log.debug(String.format("before adjust self left=%s right=%s span=%s",
		...
		session.createQuery(hql).setParameter("offset", offset).setParameter(
				...
				.executeUpdate();
		if (log.isDebugEnabled()) {
			log.debug(String.format("adjust self hql: %s, offset=%s, nodeLft=%s, nodeRgt=%s",
		...
		session.createQuery(hql2).setParameter("nodeRgt", nodeRgt)
				.executeUpdate();
		if (log.isDebugEnabled()) {
			log.debug(String.format("clear span hql1:%s, hql2:%s, nodeRgt:%s", new Object[] {
					hql1, hql2, nodeRgt }));
		}
		session.setFlushMode(model);
		return true;
